        ...
        if not st.session_state.response:
            if st.button("▶️ Analyze Video"):
                with st.spinner("Analyzing video..."):
                    ...
                    try:
                        response = requests.post(API_URL, json={"video_url": st.session_state.video_path}, timeout=300)
                    except requests.exceptions.ConnectionError:
                        st.error("🔌 Cannot connect to the backend server. Please make sure `uvicorn main:app --reload` is running in the `backend/` directory, then try again.")
                        st.stop()
                    except requests.exceptions.Timeout:
                        st.error("⏱️ The request timed out. The video may be too long or the server is busy.")
                        st.stop()
                    ...
                    if response.status_code == 200:
                        try:
                            ...
                            if isinstance(result, dict):
                                ...
                                st.session_state.facial_expression_response = result.get("facial_expression_response")
                                st.session_state.voice_analysis_response = result.get("voice_analysis_response")
                                st.session_state.content_analysis_response = result.get("content_analysis_response")
                                st.session_state.feedback_response = result.get("feedback_response")
                                st.success("Video analysis completed successfully.")
                                st.rerun()
                            else:
                                st.error("🚨 Unexpected response format from server.")
                        except Exception as e:
                            st.error(f"🚨 Failed to parse analysis results: {e}")
                    else:
...
with col2:
    ...
    if st.session_state.response:
        # Priority 1: New direct transcription field from API
        if st.session_state.response.get("transcription"):
            transcript = st.session_state.response.get("transcription")
        # Priority 2: Nested in voice_analysis_response
        elif st.session_state.voice_analysis_response:
            try:
                var = st.session_state.voice_analysis_response
                if isinstance(var, str):
                    transcript = json.loads(var).get("transcription")
                elif isinstance(var, dict):
                    transcript = var.get("transcription")
            except Exception:
                transcript = "Error parsing transcript from nested data."
    else:
        transcript = None
